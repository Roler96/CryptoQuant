...
8. 内部因果和现货记账测试不能替代真实成交、容量与执行偏差验证。
...
## 后续独立循环：波动率管理现货（VMS v1）—— 换掉零假设而非换信号

把前 44 个失败放在一起看，死因是**同一个**：它们都在**择时**进出一个 DOGE 多头仓位，
因此都倒在**匹配随机入场零假设**上。这指向一个结构性事实——DOGE 收益由少数不可预测
的右尾暴涨主导，"择时"无法跑赢"随机在场"。所以本轮不再提第 45 个择时信号，而是提出
唯一一类**在构造上免疫于该死因**的策略：**永远做多、从不择时进出，只调整仓位大小**
（波动率管理，Moreira & Muir）。匹配随机入场零假设对它不适用，主检验因而改为
**VMS 对恒定满仓 buy-and-hold 的 Sharpe 差**（联合 circular block bootstrap）。

机制：逆波动率定仓，`w = clip(median(rv)/rv, 0, 1)`，20 日 rv、180 日中位基线、5%
量化，仅 long、权重∈[0,1]、不杠杆。协议、实现和14项定向测试在读取结果前完成，硬截止
`2024-01-01`，全部对比在共同 post-warmup 窗口（bar≥386）上做。

结果 **DISCOVERY FAIL**，但它是整个体系里信息量最大的一次：

| Candidate | Return | Sharpe | MaxDD | Avg exp | Turnover |
|---|---:|---:|---:|---:|---:|
| vms（主） | -12.54% | 0.26 | -60.31% | 89% | 8.5x |
| anti（placebo） | -40.94% | 0.07 | -67.26% | 85% | 13.9x |
| buy_and_hold（基准） | -36.92% | 0.16 | -69.35% | 100% | 0.0x |

**微妙之处：VMS 实际上"赢了方向、输了统计"。** 三个软门通过——Sharpe 0.26 > BH 0.16、
MaxDD 60.3% < BH 69.4%、且优于 anti 反向对照（0.26 > 0.07）。即减仓确实在这段样本上
同时改善了 Sharpe 与回撤，方向与 Moreira-Muir 一致。**但它没有通过证据门**：

1. **联合 block-bootstrap 单侧 `p=0.317`**（Sidak 后 `p=1.0`）—— Sharpe 差
   `+0.104` 完全在噪声内（bootstrap P5–P95 = `-0.211 .. +0.392`，跨越 0）。三年只有
   2022（同跌少亏）满足 `Sharpe(VMS)>Sharpe(BH)`；
2. **右尾风险被证实**：2021 DOGE 暴涨 `+2890%`（Sharpe 1.75），VMS 因高波动减仓只吃到
   `+2.95%`——协议预注册的"减仓砍掉产生收益的右尾"这一反面风险**真实发生了**，这正是
   VMS 三年只有 1 年跑赢 Sharpe 的原因；
3. 毛收益（0 成本）为 `-11.44%`，扣成本 `-12.54%`，换手仅 8.5x，**不是被成本吃掉**。

**这次 FAIL 的价值高于前 44 次的任何一次**：它是第一个**方向正确、机制成立**（减仓
降回撤、逆波动优于反向对照）却仍因**样本内 Sharpe 改善不显著**而被诚实拒绝的候选。
它同时钉死了两件事——(a) DOGE 上"波动率管理降回撤"是真的，但 (b) 它换来的代价是系统
性地错过与高波动同来的右尾暴涨，两者在这段历史上净额抵消到统计不可分。deflated Sharpe
`0.029` 进一步确认：49 次尝试的背景下，这个 Sharpe 毫无异常性。

按协议 VMS v1 归档为 `DISCOVERY FAIL`、**不打开 2024**、不挑邻域替补、不改 `size_cap`
再战，永久 `REJECTED`。累计尝试账本由 44 增至 **49**。

- 冻结协议：`VOLATILITY_MANAGED_SPOT_PROTOCOL_2026-07-22.md`
- 实现：`research/explore_doge_volatility_managed_spot.py`（SHA256
  `4ee7c0350adacd2977368daf8633ed5e91b257619f635c31cf7f1d0c2b263ba8`）
- 完整结果：`reports/research/doge_volatility_managed_spot_discovery.json`
- 自动报告：`VOLATILITY_MANAGED_SPOT_DISCOVERY_RESULTS_2026-07-22.md`

加入 VMS 后全量回归更新为 **534 passed**；全仓 Ruff 通过，VMS 定向 Pyright（注入项目
venv）0 errors。

### 由 45–49 号尝试得到的方法论结论

DWS 与 VMS 一起把搜索空间的两端都探到了：DWS 证明**纯日历**无信息，VMS 证明**纯风险
缩放**在样本内不可证显著且会牺牲右尾。结合此前 44 次择时失败，当前对 DOGE 现货**单资产、
仅历史价格**的证据是：既没有可发现的择时 alpha，也没有可证显著的波动率定仓 edge。因此
后续应把新证据放到 `2026-07-20` 冻结点之后的 **forward-only** 数据上（如 OIFR 路线），
不再在已被反复消费的 2021–2025 样本上寻找第 50 个历史公式。
