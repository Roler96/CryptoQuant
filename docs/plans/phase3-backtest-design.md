...
**当前设计**：策略只输出信号序列 `[1, -1, 0, ...]`，价格逻辑完全由引擎控制：
- 入场价 = 下一根 K 线的开盘价
- 止损价 = 入场价 × (1 - stop_loss_pct)
- 止盈价 = 入场价 × (1 + take_profit_pct)

所有策略共享同一套固定百分比止损止盈逻辑。

**局限性**：
1. 无法实现 ATR 动态止损（止损幅度随波动率变化）
2. 无法实现限价单入场（策略指定入场价格）
3. 无法实现追踪止损（止损价随盈利移动）
4. 无法实现基于市场结构的出场（如支撑/阻力位）
...
```python
# 策略可以返回元组
def generate_signal(self, df) -> tuple[pd.Series, pd.Series, pd.Series]:
    """返回 (signal, entry_price, stop_loss_price)"""
    ...
```
- 优点：灵活，策略可以控制入场价和止损价
- 缺点：接口复杂，向后兼容问题
...
```python
class Strategy(ABC):
    def calculate_levels(self, entry_price: float, df: pd.DataFrame) -> dict | None:
        """可选：策略自定义止损止盈价位。
...
**决策状态**：⏸️ 待定。当前保持方案 A，后续根据策略需求再决定是否演进。
