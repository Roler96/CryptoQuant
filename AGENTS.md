...
2. **校准闸门 CLOSED**：见 `docs/engine_calibration_gate.md`。闸门打开前**不产出任何
   研究结论/绩效数字**。
3. **无前视架构**：策略只见 `Context` 游标切片；信号读闭合 bar `t`，`t+1` open 成交；
   多周期门控由 `core/clock.py` as-of 对齐。现货 `target < 0` 直接 raise。
4. **衍生数据浅历史**：funding/OI 只有数月真实深度，**绝不 forward-fill 到早期年份**；
...
- **strategy-validator** — 见 [`.agents/strategy-validator.md`](.agents/strategy-validator.md)。
  proposer 的对手方，做**方法论审计**（同样不出数字，闸门兼容）：模式 A 预注册裁判把提案判为
...
- **strategy-backtester** — 见 [`.agents/strategy-backtester.md`](.agents/strategy-backtester.md)。
  **门感知执行器**：把 FIT 协议实现成 `cq/strategy/*.py` 并在引擎上跑。闸门 CLOSED 时只做工程活
  （实现+单测、paper↔backtest 对拍、干跑校验，裁决栏 `GATE-BLOCKED`）；闸门 OPEN 后才跑
  discovery+validation 出 RESULTS，再交回 validator 取证。
...
→ **backtester** 实现并执行（闸门 CLOSED 时只工程对拍、开后出 RESULTS）→ **validator** 取证审计 RESULTS。
