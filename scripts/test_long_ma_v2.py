...
test_cases = [
    # Long MAs - should capture trends with fewer trades
    {'name': 'SMA_50_200_nostop', 'params': {'fast_ma_period': 50, 'slow_ma_period': 200, 'ma_type': 'sma', 'use_stop_loss': False}},
    {'name': 'SMA_30_120_nostop', 'params': {'fast_ma_period': 30, 'slow_ma_period': 120, 'ma_type': 'sma', 'use_stop_loss': False}},
    {'name': 'EMA_50_200_nostop', 'params': {'fast_ma_period': 50, 'slow_ma_period': 200, 'ma_type': 'ema', 'use_stop_loss': False}},
    # Medium MAs with wider stops
    {'name': 'SMA_20_50_wide', 'params': {'fast_ma_period': 20, 'slow_ma_period': 50, 'ma_type': 'sma', 'stop_loss_pct': 0.10, 'take_profit_pct': 0.30}},
    # Default CTA on TON for comparison
]
...
for r in results:
    ...
    print(f"{marker}{r['params']:25s} | {r['pair']:12s} | {r['return']:>+7.1%} | {r['trades']:4d} | L:{r['long']:3d} S:{r['short']:3d} | PF:{r['pf']:.2f} | WR:{r['wr']:.0f}% | MDD:{r['mdd']:.1f}%")
