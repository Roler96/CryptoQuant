"""Tests for CMFTrend strategy."""
import numpy as np
import pandas as pd
import pytest

from cryptoquant.exceptions import StrategyError
from research.backtest_cmf_trend import CMFTrend
...
def _make_uptrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with uptrend + accumulation (close near high) for CMF long."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
    ...
    # Accumulation pattern: close near high, high volume
    volume = np.full(n, 5000.0)
    high = close + 0.3   # tight range → close near high → positive MFM
    low = close - 1.8
    return pd.DataFrame(
        {
            "open": close - 0.2,
...
def _make_downtrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with downtrend + distribution (close near low) for CMF short."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
    rng = np.random.default_rng(42)
    ...
    # Distribution pattern: close near low, high volume
    volume = np.full(n, 5000.0)
    high = close + 1.8
    low = close - 0.3   # tight range → close near low → negative MFM
    return pd.DataFrame(
        {
            "open": close + 0.2,
            "high": high,
...
class TestCMFTrend:
    ...
    def test_custom_params_instantiation(self):
        ...
        s = CMFTrend(params={"cmf_period": 14, "trend_period": 100})
        assert s.params["cmf_period"] == 14
    ...
    def test_uptrend_produces_long_signals(self):
        """Uptrend with accumulation should produce long signals."""
        df = _make_uptrend_df(500)
        s = CMFTrend()
        signal = s.generate_signal(df)
        assert (signal == 1).any()
    ...
    def test_downtrend_produces_short_signals(self):
        """Downtrend with distribution should produce short signals."""
