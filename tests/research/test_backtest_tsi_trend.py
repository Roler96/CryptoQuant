"""Tests for TSITrend strategy."""
import numpy as np
import pandas as pd
import pytest

from cryptoquant.exceptions import StrategyError
from research.backtest_tsi_trend import TSITrend
...
def _make_strong_uptrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with strong uptrend to trigger TSI long entry."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
...
class TestTSITrend:
    ...
    def test_custom_params_instantiation(self):
        ...
        s = TSITrend(
            params={"tsi_short": 10, "tsi_long": 20, "trend_period": 100}
        )
        assert s.params["tsi_short"] == 10
        assert s.params["tsi_long"] == 20
