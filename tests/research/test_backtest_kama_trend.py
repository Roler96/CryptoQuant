"""Tests for KamaTrend strategy."""
import numpy as np
import pandas as pd
import pytest

from cryptoquant.exceptions import StrategyError
from research.backtest_kama_trend import KamaTrend
...
def _make_strong_uptrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with strong uptrend to trigger KAMA long entry."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
...
class TestKamaTrend:
    ...
    def test_custom_params_instantiation(self):
        ...
        s = KamaTrend(
            params={"er_period": 15, "slow_ema_slow": 60, "trend_period": 100}
        )
        assert s.params["er_period"] == 15
        assert s.params["slow_ema_slow"] == 60
