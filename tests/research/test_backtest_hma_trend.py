"""Tests for HMATrend strategy."""
import numpy as np
import pandas as pd
import pytest

from cryptoquant.exceptions import StrategyError
from research.backtest_hma_trend import HMATrend
...
def _make_strong_uptrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with strong uptrend and expanding bars."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
    ...
    # Markedly different highs/lows for ATR expansion
...
class TestHMATrend:
    ...
    def test_custom_params_instantiation(self):
        ...
        s = HMATrend(
            params={"hma_fast": 10, "hma_slow": 30, "expansion_mult": 2.0}
        )
        assert s.params["hma_fast"] == 10
        assert s.params["hma_slow"] == 30
    ...
    def test_uptrend_produces_long_signals(self):
        """Strong uptrend with expansion should produce long signals."""
        df = _make_strong_uptrend_df(500)
        s = HMATrend()
        signal = s.generate_signal(df)
        assert (signal == 1).any()
    ...
    def test_downtrend_produces_short_signals(self):
        """Strong downtrend with expansion should produce short signals."""
