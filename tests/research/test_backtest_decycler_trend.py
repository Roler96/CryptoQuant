"""Tests for DecyclerTrend strategy."""
...
from research.backtest_decycler_trend import DecyclerTrend
...
def _make_strong_uptrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with strong uptrend for Decycler crossover testing."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
...
class TestDecyclerTrend:
    ...
    def test_custom_params_instantiation(self):
        ...
        s = DecyclerTrend(params={"cutoff_period": 14, "trend_period": 150})
        assert s.params["cutoff_period"] == 14
        assert s.params["trend_period"] == 150
