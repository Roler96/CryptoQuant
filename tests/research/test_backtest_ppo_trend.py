...
from research.backtest_ppo_trend import PPOTrend
...
def _make_strong_uptrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with strong uptrend + accelerating returns for PPO cross."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
    ...
    base = 100.0
    for i in range(n):
        # Accelerating returns to trigger PPO cross above signal
        if i < n * 0.4:
            base += 0.05  # Slow initial phase
        else:
            base += 0.15  # Acceleration phase
...
class TestPPOTrend:
    """Tests for the PPOTrend strategy."""

    def test_default_params_instantiation(self):
        """Strategy can be instantiated with default parameters."""
        s = PPOTrend()
        assert s.name == "PPOTrend"
        assert s.timeframe == "1h"
        assert s.min_bars == 250
        assert s.version == "1.0.0"
        assert s.params["ppo_fast"] == 12
        assert s.params["ppo_slow"] == 26
        assert s.params["ppo_signal"] == 9
    ...
    def test_custom_params_instantiation(self):
        ...
        assert s.params["ppo_fast"] == 5
        assert s.params["ppo_slow"] == 20
    ...
    def test_flat_data_no_signals_after_warmup(self):
        """Flat data should produce minimal signals after warmup (PPO near zero)."""
        df = _make_flat_df(500)
        s = PPOTrend()
        signal = s.generate_signal(df)
        # After warmup, flat data should have very few signal changes
        non_warmup = signal.iloc[280:]
        unique_in_window = set(non_warmup.unique())
        # Mostly zero or minimal signals
        assert 0 in unique_in_window
