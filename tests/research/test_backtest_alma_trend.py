"""Tests for ALMATrend strategy."""
import numpy as np
import pandas as pd
import pytest

from cryptoquant.exceptions import StrategyError
from research.backtest_alma_trend import ALMATrend
...
def _make_strong_uptrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with strong uptrend for ALMA crossover testing."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
...
class TestALMATrend:
    """Tests for the ALMATrend strategy."""

    def test_default_params_instantiation(self):
        """Strategy can be instantiated with default parameters."""
        s = ALMATrend()
        assert s.name == "ALMATrend"
        assert s.timeframe == "1h"
        assert s.min_bars == 235
        assert s.version == "1.0.0"
        assert s.params["alma_fast"] == 9
        assert s.params["alma_slow"] == 30
        assert s.params["alma_offset"] == 0.85
        assert s.params["alma_sigma"] == 6.0
    ...
    def test_custom_params_instantiation(self):
        ...
        s = ALMATrend(params={"alma_fast": 5, "alma_slow": 20, "alma_offset": 0.7})
        assert s.params["alma_fast"] == 5
        assert s.params["alma_slow"] == 20
        assert s.params["alma_offset"] == 0.7
