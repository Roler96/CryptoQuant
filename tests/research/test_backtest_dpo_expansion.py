"""Tests for DPOExpansion strategy."""
import numpy as np
import pandas as pd
import pytest

from cryptoquant.exceptions import StrategyError
from research.backtest_dpo_expansion import DPOExpansion
...
def _make_uptrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with strong uptrend + rising volatility for entry trigger."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
    ...
    for i in range(n):
        base += 0.06
        # Gradually increase volatility to trigger ATR expansion
        volatility = min(volatility + 0.002, 3.0)
