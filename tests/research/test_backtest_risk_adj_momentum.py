"""Tests for RiskAdjustedMomentum strategy."""
import numpy as np
import pandas as pd
import pytest

from cryptoquant.exceptions import StrategyError
from research.backtest_risk_adj_momentum import RiskAdjustedMomentum
...
def _make_strong_uptrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with strong uptrend to trigger risk-adj momentum entry.

    Strong uptrend (close rises 50% over n bars) keeps risk-adj momentum
    well above threshold. Price stays above EMA200 after initial warmup.
    """
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
    ...
    base = 100.0
    for i in range(n):
        base += 0.12  # steady climb (~60 points over 500 bars)
...
class TestRiskAdjustedMomentum:
    """Tests for the RiskAdjustedMomentum strategy."""

    def test_default_params_instantiation(self):
        """Strategy can be instantiated with default parameters."""
        s = RiskAdjustedMomentum()
        assert s.name == "RiskAdjustedMomentum"
        assert s.timeframe == "1h"
        assert s.min_bars == 300
        assert s.version == "1.0.0"
        assert s.params["momentum_period"] == 63
    ...
    def test_custom_params_instantiation(self):
        ...
        s = RiskAdjustedMomentum(
            params={"momentum_period": 30, "vol_period": 14, "threshold": 0.8}
        )
        assert s.params["momentum_period"] == 30
        assert s.params["vol_period"] == 14
