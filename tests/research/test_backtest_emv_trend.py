"""Tests for EMVTrend strategy."""
import numpy as np
import pandas as pd
import pytest

from cryptoquant.exceptions import StrategyError
from research.backtest_emv_trend import EMVTrend
...
def _make_uptrend_df(n: int = 500) -> pd.DataFrame:
    """Create trending data with increasing price + volume."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
...
class TestEMVTrend:
    ...
    def test_initialization(self):
        ...
        s = EMVTrend({})
        assert s.name == "EMVTrend"
        assert s.params["emv_smooth"] == 5
        assert s.params["trend_period"] == 200

    def test_custom_params(self):
        """Custom params override defaults."""
        s = EMVTrend({"emv_smooth": 10, "trend_period": 100})
    ...
    def test_flat_price_no_signals(self):
        """Flat price produces valid-range signals."""
        s = EMVTrend({})
