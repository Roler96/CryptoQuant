"""Tests for ADXSlopeTrend strategy."""
import numpy as np
import pandas as pd
import pytest

from cryptoquant.exceptions import StrategyError
from research.backtest_adx_slope import ADXSlopeTrend
...
def _make_strong_uptrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with strong uptrend to trigger ADX slope positive."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
    ...
    base = 100.0
    for i in range(n):
        base += 0.15  # sufficient trend to build ADX
...
class TestADXSlopeTrend:
    ...
    def test_custom_params_instantiation(self):
        ...
        s = ADXSlopeTrend(params={"adx_period": 20, "trend_period": 100})
        assert s.params["adx_period"] == 20
