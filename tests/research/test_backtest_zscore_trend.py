"""Tests for ZScoreTrend strategy."""
...
from research.backtest_zscore_trend import ZScoreTrend
...
def _make_strong_uptrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with strong steady uptrend to trigger Z-score long."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
    ...
    base = 100.0
    for i in range(n):
        base += 0.12  # Steady positive returns to push Z-score above 0
...
def _make_strong_downtrend_df(n: int = 500) -> pd.DataFrame:
    """Create data with strong steady downtrend for short entry testing."""
    dates = pd.date_range("2024-01-01", periods=n, freq="1h")
    rng = np.random.default_rng(42)
    ...
    base = 200.0
    for i in range(n):
        base -= 0.12  # Steady negative returns
        noise = rng.normal(0, 0.2)
        close[i] = base + noise
...
class TestZScoreTrend:
    """Tests for the ZScoreTrend strategy."""

    def test_default_params_instantiation(self):
        """Strategy can be instantiated with default parameters."""
        s = ZScoreTrend()
        assert s.name == "ZScoreTrend"
        assert s.timeframe == "1h"
        assert s.min_bars == 300
        assert s.version == "1.0.0"
        assert s.params["zscore_short"] == 20
        assert s.params["zscore_long"] == 100
    ...
    def test_custom_params_instantiation(self):
        ...
        assert s.params["zscore_short"] == 10
        assert s.params["zscore_long"] == 50
    ...
    def test_no_signals_on_flat_data(self):
        """Flat price data should produce zero signals after warmup."""
        df = _make_flat_df(500)
        s = ZScoreTrend()
        signal = s.generate_signal(df)
        # Flat data = zero returns → Z-score near 0 → no cross → all 0
        non_warmup = signal.iloc[250:]  # After warmup
        assert (non_warmup == 0).all()
