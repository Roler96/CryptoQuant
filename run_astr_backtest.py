"""Backtest DogeSpotDonchianSma on ~1 year of ASTR/USDT 4h data (Binance)."""
import sys
import time
...
def fetch_data() -> pd.DataFrame:
    """Fetch ASTR/USDT 4h data, using store as cache."""
    store = OHLCVStore()
    ...
    if not df.empty and len(df) > 2000:
        ...
        if latest and latest > END_MS - 86400000:
            store.close()
            return df
...
def main():
    ...
    # Binance spot: 0.1% taker, ~0.05% slippage
